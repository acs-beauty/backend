...
const findAllCategoryAndSub = async isAdmin => {
  ...
  const count = isAdmin ? [[Sequelize.literal(`CAST(COUNT("subcategories->products") AS INTEGER)`), 'productCount']] : []
  ...
  try {
    const categoryAndSub = await Category.findAll({
      ...
      include: {
        model: Subcategory,
        as: 'subcategories',
        attributes: ['subcategoryId', 'name', 'linkKey', ...attributeDisabled, ...count],
        required: !isAdmin,
        where: { ...whereDisabled },
        include: {
          model: Product,
