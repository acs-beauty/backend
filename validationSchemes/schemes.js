...
module.exports.queryStringSchema = yup.object().shape({
  ...
  productIds: yup.string().matches(regex.arrayNumbersString, 'Invalid productIds format'),
})
...
module.exports.searchProductsSchema = yup.object().shape({
  limit: yup.number().positive(),
  offset: yup.number().min(0),
  searchWords: yup.string().required().matches(regex.arraySearchProductString, 'Invalid searchWords format'),
})
...
const linkKeyRequired = {
  linkKey: yup
    ...
    .matches(regex.validateSlug, 'Invalid linkKey format'),
}
...
module.exports.authSchema = yup.object().shape({
  login: yup.string().required().matches(regex.emailRegex, 'Invalid email format'),
  password: yup.string().required().matches(regex.passwordRegex, 'Invalid password format'),
})
